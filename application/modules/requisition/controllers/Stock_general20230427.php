     ...
     public function index($dt='')
    {
      ...
      if($dt){
        ...
        $condv="";
                ...
                $resultat=$this->Model->getRequete("SELECT p.*, ((SELECT IFNULL(SUM(QUANTITE),0) from req_requisition where ID_PRODUIT=p.ID_PRODUIT ".$cond.")-(SELECT COUNT(ID_VENTE_DETAIL) from vente_vente v join vente_detail vd on v.ID_VENTE=vd.ID_VENTE where ID_PRODUIT=p.ID_PRODUIT  ".$condv.")-(SELECT IFNULL(SUM(QUANTITE),0) from req_stock_disparu where ID_PRODUIT=p.ID_PRODUIT ".$cond.")-(SELECT COUNT(ID_STOCK_ENDOMAGE) from req_stock_endomage where ID_PRODUIT=p.ID_PRODUIT ".$cond.")) as NOMBRE, (SELECT MIN(PRIX_ACHAT_UNITAIRE) from req_requisition where ID_PRODUIT=p.ID_PRODUIT ".$cond.") PRIX  FROM saisie_produit p WHERE 1   ORDER BY NOM_PRODUIT");
       $tabledata=array();
       $i=1;
