 ...
 class Declassement extends CI_Controller
 {
  ...
  public function save_declassement()
  {
    ...
    $this->load->view("Declassement_Add_View",$data);
        ...
        $this->Model->update('req_barcode',array('ID_BARCODE'=>$this->input->post('ID_BARCODE')),array('STATUS'=>2));
  ...
  public function listing()
    {
      ...
      if ($DATE_DEBUT != NULL) {
        ...
        if ($DATE_FIN != NULL) {
          ...
          $data['conddatedebut'] = ' AND DATE_TIME BETWEEN "'.$DATE_DEBUT.'" AND "'.$DATE_FIN.'"';
        }
        else{
          $data['DATE_DEBUTS']= $this->input->post('DATE_DEBUT');
          $data['DATE_FINS']= date("Y-m-d");
          $data['conddatedebut'] = ' AND DATE_TIME BETWEEN "'.$DATE_DEBUT.'" AND "'.date("Y-m-d").'"';
        }
          ...
          $data['DATE_FINS']= date("Y-m-d");
          $data['conddatedebut'] = ' AND DATE_TIME BETWEEN "'.date("Y-m-d").'" AND "'.date("Y-m-d").'"';
      }
