<?php 
 ...
 class Entree_Stock extends CI_Controller
 {
  ...
  public function save_scan()
  {
    ...
    $unique = $this->Model->getRequeteOne('SELECT HAVE_FACTURE,ID_REQUISITION,DATE_REQUISITION,PRIX_VENTE_UNITAIRE,req_requisition.ID_PRODUIT,saisie_produit.NOM_PRODUIT,QUANTITE FROM `req_requisition` JOIN saisie_produit ON saisie_produit.ID_PRODUIT = req_requisition.ID_PRODUIT WHERE 1 AND ID_REQUISITION = '.$this->input->post('ID_REQUISITION').' AND saisie_produit.ID_PRODUIT = '.$this->input->post('ID_PRODUIT').'  ');
     ...
     $data_qr = array(
               ...
               'PRIX_VENTE'=>$this->input->post('PRIX_VENTE'),
               'STATUS'=>1,
               'HAVE_FACTURE'=>$unique['HAVE_FACTURE'],
     ...
     mb_strlen("Hello world. The world is nice","world");
     // print_r($data_qr);

     // split('-', $this->input->post('BARCODE'))
     $new = explode("-",$this->input->post('BARCODE'));
     // echo $new[0];
     // echo "<br>";
     // echo $new[1];
     // echo "<br>";
        ...
        if (isset($new[0])) {
            ...
            if (is_numeric($new[0])) {
                 ...
                 if (isset($new[1])) {
                    // echo "Variable 1 is set.".$new[0]." and good<br>";
                    if (is_numeric($new[1])) {
                        echo "Variable 2 is set.".$new[1]." and good<br>";

                    }
                        else{
                            $message = "<div class='alert alert-danger' role='alert' id='message'>
                       Mauvais format du Barre code
     ...
     if ($exist) {
        ...
        if ($pexist) {
           ...
           $datacr = array('QUANTITE'=>$tupdate['QUANTITE']+1,'STATUS'=>1,"ENVOIE"=>0);
  ...
  public function annuler_action($ID_BARCODE)
  {
    ...
    if (!empty($pexist)) {
        ...
        $data_qr = array(
               ...
               'STATUS'=>1,
               'ID_SOCIETE'=>$this->session->userdata('STRAPH_ID_SOCIETE'),"ENVOIE"=>0
              );
    ...
    //SI Valide
     if ($this->form_validation->run() == FALSE){        
       ...
       $this->load->view("Requisition_Update_View",$data);
        ...
        $this->Model->update('voiture_carburant_consomation',array('CONSOMATION_ID'=>$CONSOMATION_ID),array('ID_STATUS'=>2,'USERCHANGER'=>$this->session->userdata('STRAPH_ID_USER'),"ENVOIE"=>0 ));
  ...
  public function suppimer()
  {
    ...
    // exit();
// annuler suppimer
    $detail = $this->Model->update('voiture_carburant_consomation',array('CONSOMATION_ID'=>$CONSOMATION_ID),array('ID_STATUS'=>3,'USERDELETE'=>$this->session->userdata('STRAPH_ID_USER'),"ENVOIE"=>0));
  ...
  public function annuler()
  {
    ...
    // exit();
// annuler suppimer
    $detail = $this->Model->update('voiture_carburant_consomation',array('CONSOMATION_ID'=>$CONSOMATION_ID),array('ID_STATUS'=>4,'USERANNULE'=>$this->session->userdata('STRAPH_ID_USER'),"ENVOIE"=>0));
  ...
  public function paye()
  {
    ...
    // exit();
// annuler suppimer
    $detail = $this->Model->update('voiture_carburant_consomation',array('CONSOMATION_ID'=>$CONSOMATION_ID),array('IS_PAYE'=>1,'USERPAYE'=>$this->session->userdata('STRAPH_ID_USER'),"ENVOIE"=>0));
