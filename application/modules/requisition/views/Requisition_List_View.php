    ...
    <section class="content">
      <div class="container-fluid">
            ...
            <div class="card card-success">
              ...
              <div class="card-body">
                ...
                <label for="exampleInputName2">Periode:</label>
                      ...
                      <input type="text" class="form-control float-right" name="PERIODE" value="<?php echo $selectdate; ?>" id="reservation" onchange="changelist()" >  
                    </div> -->

                    <div class="form-group col-md-3 col-sm-3 col-xs-3">
                        <label for="exampleInputName2">Fournisseur:</label>
                        <select class="form-control select select2-success" data-dropdown-css-class="select2-success" onchange="changelist()" name="ID_FOURNISSEUR" id="ID_FOURNISSEUR" style="width: 100%;">
                        <option value=""> </option>
                        <?php 
                    ...
                    <div class="form-group col-md-3 col-sm-3 col-xs-3">
                      ...
                      <select class="form-control select select2-success" data-dropdown-css-class="select2-success" onchange="changelist()" name="ID_USER_SAISIE" id="ID_USER_SAISIE" style="width: 100%;">
                                      <option value=""> </option>
                                      <?php 
        ...
        <tbody>
          ...
          $resultat=$this->Model->getRequete('SELECT DATE_REQUISITION, ID_USER_REQUISITION, saisie.NOM AS NOMS, saisie.PRENOM AS PRENOMS, ID_USER_SAISIE, config_user.NOM, config_user.PRENOM, req_requisition.ID_FOURNISSEUR, saisie_fournisseur.NOM AS FOURNISSEUR, SUM(MONTANT_TOTAL_ACHAT) AS MONTANT_TOTAL_ACHAT FROM `req_requisition` JOIN config_user ON config_user.ID_USER = req_requisition.ID_USER_REQUISITION JOIN saisie_fournisseur ON saisie_fournisseur.ID_FOURNISSEUR = req_requisition.ID_FOURNISSEUR JOIN config_user as saisie ON saisie.ID_USER = req_requisition.ID_USER_SAISIE WHERE req_requisition.ID_SOCIETE = '.$this->session->userdata('STRAPH_ID_SOCIETE').' '.$condfournisseur.' '.$conduser.' '.$condusersa.'  '.$conddatedebut.' GROUP BY DATE_REQUISITION , ID_USER_REQUISITION, ID_FOURNISSEUR, config_user.NOM, PRENOM ,saisie_fournisseur.NOM,ID_USER_SAISIE, saisie.NOM ,  saisie.PRENOM ');
         $tot = 0;
         ...
         echo "<td>
         <a class='btn btn-success btn-sm' href='". base_url('requisition/Requisition/detail/'.$key['DATE_REQUISITION'].'/'.$key['ID_USER_REQUISITION'].'/'.$key['ID_FOURNISSEUR'].'/'.$key['ID_USER_SAISIE'])."' role='button'>Details</a> 
         </td></tr>";
