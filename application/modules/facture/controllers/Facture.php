...
# code...
  ...
  $exists = $this->db->get_where('facture', array('ID_ASSURANCE' => $ID_ASSURANCE,'MOIS' => $MOIS));
  
   if($exists->num_rows() > 0 ){
     $data['message']='<div class="alert alert-danger text-center" id="message">'."Cette Assurance Avec Cette Date Existe Déjà".'</div>';
  $this->session->set_flashdata($data);
       
    redirect(base_url() . 'facture/Facture/inserting');
   }else{
