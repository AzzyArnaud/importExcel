<?php 
  ...
  * 
  */
 class Vente_List_Dette extends CI_Controller
 {
  ...
  public function submitpaiement()
  {
     ...
     $montant = $this->Model->getRequeteOne('SELECT MONTANT_PAYE  FROM `vente_vente` WHERE 1 AND `ID_VENTE` = '.$ID_VENTE.'  GROUP BY MONTANT_PAYE');
     $montantpaye = $this->Model->getRequeteOne('SELECT SUM(MONTANT_PAYE) AS MONTANT_PAYE  FROM `vente_paiement_dette` WHERE 1 AND `ID_VENTE` = '.$ID_VENTE.'  GROUP BY MONTANT_PAYE');

     // if (empty($montantpaye)) {
     //   $reste = $montant['MONTANT_PAYE'];
     // }
     // else{
      $reste = $montant['MONTANT_PAYE'] - $montantpaye['MONTANT_PAYE'] - $MONTANT_PAYE;
     // }
     ...
     if ($reste == 0) {
     $this->Model->create('vente_paiement_dette',$data_qr);
     $this->Model->update('vente_vente',array('ID_VENTE'=>$ID_VENTE),array('IS_DETTE'=>0,"ENVOIE"=>0));
     echo "Paiement de la dette effectue avec succès";
     }
     else{
      if ($reste > 0) {
         echo "Montant non paye totalement";
         $this->Model->create('vente_paiement_dette',$data_qr);
      }
      else{
         echo "Montant depasse paiement non enregistre";
      }
