...
// $newDate = date("d-m-Y", strtotime($originalDate));
      ...
      if ($ID_USER !=NULL) {
        ...
        $vendeurs = $this->Model->getRequeteOne('SELECT config_user.NOM, config_user.PRENOM FROM config_user WHERE ID_USER = '.$ID_USER.' ORDER BY ID_USER');
        $data['vendeurs'] = $vendeurs['NOM'].' '.$vendeurs['PRENOM'];
      }
      else{
        $conduser='';
        $data['vendeurs'] = 'Tout le monde';
      }
     ...
     $data['vente'] = $this->Model->getRequete('SELECT ID_USER_VENDEUR, config_user.NOM, config_user.PRENOM,`DATE_TIME_VENTE`,  `MONTANT_TOTAL`, `MONTANT_PAYE`, `MONTANT_REMISE`, saisie_client.NOM_CLIENT, saisie_client.PRENOM_CLIENT, `ID_VENTE`, vente_vente.IS_LIVRAISON, vente_vente.IS_DETTE FROM `vente_vente` JOIN config_user ON config_user.ID_USER = vente_vente.ID_USER_VENDEUR LEFT JOIN saisie_client ON saisie_client.ID_CLIENT = vente_vente.ID_CLIENT WHERE 1 AND vente_vente.ID_SOCIETE = '.$this->session->userdata('STRAPH_ID_SOCIETE').' '.$conduser.' '.$conddatedebut.' order by ID_VENTE DESC');
     $sommes = $this->Model->getRequeteOne('SELECT SUM(MONTANT_TOTAL) AS MONTANT_TOTAL, SUM(MONTANT_PAYE) AS MONTANT_PAYE, SUM(MONTANT_REMISE) AS MONTANT_REMISE FROM `vente_vente` JOIN config_user ON config_user.ID_USER = vente_vente.ID_USER_VENDEUR LEFT JOIN saisie_client ON saisie_client.ID_CLIENT = vente_vente.ID_CLIENT WHERE 1 AND vente_vente.ID_SOCIETE = '.$this->session->userdata('STRAPH_ID_SOCIETE').' '.$conduser.' '.$conddatedebut.' order by ID_VENTE DESC');
     $sommes_dette = $this->Model->getRequeteOne('SELECT SUM(MONTANT_PAYE) AS MONTANT_PAYE FROM `vente_vente` JOIN config_user ON config_user.ID_USER = vente_vente.ID_USER_VENDEUR LEFT JOIN saisie_client ON saisie_client.ID_CLIENT = vente_vente.ID_CLIENT WHERE 1 AND vente_vente.ID_SOCIETE = '.$this->session->userdata('STRAPH_ID_SOCIETE').' '.$conduser.' '.$conddatedebut.' AND IS_DETTE = 1 order by ID_VENTE DESC');
     $data['MONTANT_TOTAL'] = number_format($sommes['MONTANT_TOTAL'], 0,',',' ');
     $data['MONTANT_REMISE'] = number_format($sommes['MONTANT_REMISE'], 0,',',' ');
     $data['IS_DETTE'] = number_format($sommes_dette['MONTANT_PAYE'], 0,',',' ');
     $LIQUIDITE = $sommes['MONTANT_TOTAL'] - $sommes['MONTANT_REMISE'] - $sommes_dette['MONTANT_PAYE'];
     $data['LIQUIDITE'] = number_format($LIQUIDITE, 0,',',' ');
