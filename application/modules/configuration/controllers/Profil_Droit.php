...
class Profil_Droit  extends CI_Controller{
    ...
    public function index()
    {
      ...
      $data['droits']=$this->Model->getRequete('SELECT * FROM `config_droits` WHERE 1 AND ID_SOCIETE = '.$this->session->userdata('STRAPH_ID_SOCIETE').' order by DESCRIPTION');
  ...
  $this->load->view('Profil_Droit_Add_View',$data);
    ...
    $datasprofil=array('DESCRIPTION'=>$DESCRIPTION,'ID_SOCIETE'=>$this->session->userdata('STRAPH_ID_SOCIETE'));
    // echo'<br>';
    // print_r($datasprofil);
    ...
    $this->session->set_flashdata(array('message'=>$message));
      redirect(base_url('configuration/Profil_Droit/listing'));  
   }
    ...
    public function listing()
    {
      ...
      $data['resultat']=$this->Model->getRequete('SELECT config_profil.PROFIL_ID, config_profil.DESCRIPTION, COUNT(config_profil_droit.ID_DROIT) AS NUMBER FROM `config_profil` left JOIN config_profil_droit ON config_profil_droit.PROFIL_ID = config_profil.PROFIL_ID GROUP BY config_profil.PROFIL_ID, config_profil.DESCRIPTION');
      $tabledata=array();
    ...
    public function update()
    {
      ...
      $this->load->view('Profil_Droit_Update_View',$data);
        ...
        $datasprofil=array('DESCRIPTION'=>$DESCRIPTION,"ENVOIE"=>0);
        // echo'<br>';
        // print_r($datasprofil);
    ...
    {
      $this->Model->update('config_type_conge',array('ID_TYPE_CONGE'=>$id),array('STATUS'=>0,"ENVOIE"=>0));
      $message = "<div class='alert alert-success' id='message'>
                            Type de cong&eacute; désactivé avec succés
                            <button type='button' class='close' data-dismiss='alert'>&times;</button>
                      </div>";
      $this->session->set_flashdata(array('message'=>$message));
      redirect(base_url('configuration/Type_Conge/listing'));  
    }

  public function reactiver($id)
    {
      $this->Model->update('config_type_conge',array('ID_TYPE_CONGE'=>$id),array('STATUS'=>1,"ENVOIE"=>0));
      $message = "<div class='alert alert-success' id='message'>
