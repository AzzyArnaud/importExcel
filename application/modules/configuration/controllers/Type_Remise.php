...
class Type_Remise extends CI_Controller {
  ...
  $this->form_validation->set_rules('POURCENTAGE', 'POURCENTAGE', 'required');
   ...
   $POURCENTAGE=$this->input->post('POURCENTAGE');
      ...
      $this->form_validation->set_rules('POURCENTAGE', 'POURCENTAGE', 'required');
       ...
       if ($this->form_validation->run() == FALSE){
        ...
        $datasuser=array(
                           'ID_ASSURANCE'=>$ID_ASSURANCE,
                           'POURCENTAGE'=>$POURCENTAGE,"ENVOIE"=>0
                          );
   ...
   public function reactiver($ID_TYPE_REMISE)
    {
      $this->Model->update('saisie_type_remise',array('ID_TYPE_REMISE'=>$ID_TYPE_REMISE),array('STATUS'=>1,"ENVOIE"=>0));
      $message = "<div class='alert alert-success' id='message'>
    ...
    {
      $this->Model->update('saisie_type_remise',array('ID_TYPE_REMISE'=>$ID_TYPE_REMISE),array('STATUS'=>0,"ENVOIE"=>0));
      $message = "<div class='alert alert-success' id='message'>
