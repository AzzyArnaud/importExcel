...
class User  extends CI_Controller{
  ...
  $this->form_validation->set_rules('PROFIL_ID', 'Profil', 'required',array('required'=>'le champs  est  obligatoire!!'));
   ...
   if ($this->form_validation->run() == FALSE){
      ...
      $this->form_validation->set_rules('PROFIL_ID', 'Profile', 'required',array('required'=>'le champs  est  obligatoire!!'));
       ...
       if ($this->form_validation->run() == FALSE){
        ...
        if (trim($this->input->post('PASSWORD'))!=NULL) {
          $mdp= " Avec modification du mot de passe";
           $this->Model->update('config_user',array('ID_USER'=>$ID_USER),array('PASSWORD'=>md5($this->input->post('PASSWORD')))); 
         } 
         else{
          $mdp= " Sans modification du mot de passe";
         }
          // echo $mdp;
          // exit();
        $message = "<div class='alert alert-success' id='message'>
                                Utilisateur modifi&eacute; avec succés (".$mdp.")
                                <button type='button' class='close' data-dismiss='alert'>&times;</button>
                          </div>";
        $this->session->set_flashdata(array('message'=>$message));
          redirect(base_url('configuration/User/listing'));  
    ...
    {
      $this->Model->update('config_user',array('ID_USER'=>$id),array('STATUS'=>0));
      $message = "<div class='alert alert-success' id='message'>
  ...
  public function reactiver($id)
    {
      $this->Model->update('config_user',array('ID_USER'=>$id),array('STATUS'=>1));
      $message = "<div class='alert alert-success' id='message'>
