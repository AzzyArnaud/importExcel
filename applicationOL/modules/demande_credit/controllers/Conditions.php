...
class Conditions extends CI_Controller
{
    ...
    // }
    public function index(){
          $rapport=$this->Model->getRequete('SELECT* from conditions_credit r left join utilisateur u on r.FAIT_PAR=u.ID order by r.ID_CONDITION DESC');
...
if($this->session->userdata('PROFIL')){
        ...
        // $data['option'].='<li><a href='.base_url("zone/Zone_localisation/details/").$localisation["LOCALISATION_ID"].'>Détails</li>'; 
         $data['option'].='<li style="text-align:center"><a href='.base_url("demande_credit/Conditions/update/").$val["ID_CONDITION"].'><span class="lnr lnr-pencil"style="text-alin:center"></li>';
        $data['option'].="<li style='text-align:center'><a hre='#' data-toggle='modal' 
                                  data-target='#mydelete" .$val["ID_CONDITION"]. "'><span class='lnr lnr-trash' style='color:red'></span></a></li></ul>
                  </div>
                                    <div class='modal fade' id='mydelete" .$val["ID_CONDITION"]. "'>
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-footer'>
                                                    <a class='btn btn-danger btn-md ' href='" . base_url('demande_credit/Conditions/delete/'.$val["ID_CONDITION"]). "'>Supprimer</a>
    ...
    $template=array(
      ...
      if($this->session->userdata('PROFIL')){
        $this->table->set_heading('TYPE DE CONDITION','FICHIER A TELECHARGER','AJOUTER PAR','action');
      }else
       $this->table->set_heading('TYPE DE CONDITION','FICHIER A TELECHARGER');
   ...
   $this->load->view('Conditions_View', $datas);
    }
...
$this->Model->create('conditions_credit',array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
$this->Model->create("historique_navigation",array("DESCRIPTION"=>"Ajouter une condition de crédit","USER_ID"=>$this->session->userdata('ID')));
...
$this->session->set_flashdata($data);
  redirect(base_url('demande_credit/Conditions'));
        
    }
      public function delete($id){

$rapport=$this->Model->getOne('conditions_credit',array('ID_CONDITION'=>$id));
$path=FCPATH. "uploads/rapport/".$rapport['FILE'];
      unlink($path);

    $this->Model->delete('conditions_credit',array('ID_CONDITION'=>$id));
    $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Supprimer une condition de crédit","USER_ID"=>$this->session->userdata('ID')));
...
if($this->session->userdata('PROFIL')){
    ...
    $this->load->view('Conditions_View1',$datas);
  }
...
// $data1=array('upload_data'=>$this->upload_pieceJointe->data());
$this->Model->update('conditions_credit',array("ID_CONDITION"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
$this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une condition de crédit","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }else{
                $this->Model->update('conditions_credit',array("ID_CONDITION"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FAIT_PAR'=>$this->session->userdata('ID')));
                $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une condition de crédit","USER_ID"=>$this->session->userdata('ID')));
