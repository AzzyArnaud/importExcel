...
$info_total =$this->Model->getList('actualites_evenement');
foreach ($info_total as $value) {
    ...
    $check=$this->Model->checkvalue('actualites_evenement_view',array('ADRESSE_IP'=>$ip,'ID_ACTUALITE'=>$value['ID']));
    if($check!=1){
       $this->Model->create('actualites_evenement_view',array('ADRESSE_IP'=>$ip,'ID_ACTUALITE'=>$value['ID']));
   ...
   if ($this->uri->segment(4)) {
    ...
    $query =$this->db->get('actualites_evenement','10',$segment);
    $datas['info']=$query->result();
    $datas['breadcrumb'] = $this->make_bread->output();
    // $query=$this->db->get('actualites_evenement');

    $total=$this->Model->getList('actualites_evenement'); 

    $config=array();
    $config['base_url']=base_url()."actualites/Evenement/index";
    ...
    $this->pagination->initialize($config);
        ...
        $datas['error'] = "";
            $this->load->view('Evenement_View', $datas);
...
$this->session->set_flashdata($data);
 ...
 if (!$this->upload->do_upload('fotos')) {
          ...
          $this->Model->create("actualites_evenement", array("TITRE"=>$_POST['titre'],"DESCRIPTION"=>$_POST['description'],'FAIT_PAR'=>$this->session->userdata('ID')));
          $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Ajouter une publication d'évènement'","USER_ID"=>$this->session->userdata('ID')));
        //   echo $_POST['titre'];exit();
        $this->session->set_flashdata($data);
        redirect(base_url("actualites/Evenement"));
      }else { 
       ...
       if($this->resizeImage('./uploads/actualite/'.$uploadedImage['file_name'])){
        ...
        $this->Model->create("actualites_evenement", array("TITRE"=>$_POST['titre'],"DESCRIPTION"=>$_POST['description'],"FOTO"=>$new_name."_thumb.".$ext,'FAIT_PAR'=>$this->session->userdata('ID')));
        $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Ajouter une publication d'évènement'","USER_ID"=>$this->session->userdata('ID')));
 ...
 if (!$this->upload->do_upload('fotos')) {
          ...
          $data['message']='<div class="alert alert-danger text-center"> Enregistrement sans FOTO</div>';
          $this->Model->update("actualites_evenement",array("ID"=>$id), array("TITRE"=>$_POST['titre'],"DESCRIPTION"=>$_POST['description'],'FAIT_PAR'=>$this->session->userdata('ID')));
          $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une publication d'évènement'","USER_ID"=>$this->session->userdata('ID')));
        //   echo $_POST['titre'];exit();
        $this->session->set_flashdata($data);
        redirect(base_url("actualites/Evenement"));
      }else { 
       ...
       if($this->resizeImage('./uploads/actualite/'.$uploadedImage['file_name'])){
        ...
        $this->Model->update("actualites_evenement",array("ID"=>$id), array("TITRE"=>$_POST['titre'],"DESCRIPTION"=>$_POST['description'],"FOTO"=>$new_name."_thumb.".$ext,'FAIT_PAR'=>$this->session->userdata('ID')));
        $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une publication d'évènement'","USER_ID"=>$this->session->userdata('ID')));
 ...
 $service=$this->Model->getOne('actualites_evenement',array('ID'=>$id));
  $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Ajouter une publication d'évènement'","USER_ID"=>$this->session->userdata('ID')));
...
if (!empty($service['FOTO'])) {
      ...
      $this->Model->delete('actualites_evenement',array('ID'=>$id));
...
$mot_cle=$this->input->post('mot_cle');
   ...
   if ($this->uri->segment(4)) {
    ...
    $this->load->view('Recherche_evenement_view',$data);
  }
}
?>