...
<div class='modal fade' id='srcs' tabindex="-1">
  ...
  background-size:10px 10px;}">
          <span id="msg"></span>
         <form id="myform" action="<?= base_url('actualites/Actualite/ajout') ?>" method="POST" enctype="multipart/form-data">
...
<div class="container-fluid row" >
  ...
  <div class="image-upload" style="margin-top: 20px">
    ...
    <label for="pj">
 <i class="lnr lnr-paperclip" style="font-size: 40px" align='right'></i>
...
<div class='modal fade' id='modification' tabindex="-1">
  ...
  background-size:10px 10px;}">
          <span id="msg"></span>
         <form id="myform" action="<?= base_url('actualites/Actualite/modifier') ?>" method="POST" enctype="multipart/form-data">
...
<img width="100%" src="http://media.indiatimes.in/media/photogallery/2012/Dec/best_images_of_2012_1355117665_1355117684.jpg"/></div>
  ...
  <div class="col-md-8" style="text-align: left;"><h1 style="color: #266C67">Actualités</h1></div><div class="col-md-4" style="text-align: left;">
     <form id="myform" action="<?= base_url('actualites/Actualite/recherche_actualite') ?>" method="POST" enctype="multipart/form-data">
      ...
      <div class="modal-dialog" role="document">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer justify-content-center">
            <span class="mr-4">BIJE galerie photo</span>
...
<div class="col-md-12 admin" id='<?=$value->ID?>' style="margin: 0px 0px 2px 0px; padding: 10px 10px 10px 10px; background:  #EBF0EF;">
  ...
  echo "<div id='gestion' class='row ".$value->ID."' style='display:none'><a href='' data-toggle='modal' data-target='#modification'><div  class='col-md-6'><i class='fas fa-pencil-alt' style='color:#266C67'></i></div></a><div  class='col-md-6'><a href='' id='".$value->ID."' class='delete' data-toggle='modal' data-target='#mydelete'><i class='lnr lnr-trash' style='color:red'></i></div></a></div>";
    ...
    <div class="row">
      <?php
      if($value->PIECE_JOINTE||sizeof($ftos)==1){
        ?>
    ...
    ?>
    <div id="" data-toggle="modal" data-target="#modal1<?=$value->ID?>" class="outer1 col-sm-6"  style="background:  #c2deff url('<?=base_url()?>uploads/actualite/<?=$key['FOTO']?>') ;background-size:cover;float: left;padding: 2px;background-position: center center; opacity:<?=$opacity?> ; height: 200px">
  ...
  <img class="foto" src="<?=base_url()?>uploads/actualite/<?=$key['FOTO']?>" width="100%" height="100%" >
    ...
    ?>
    <div class="col-md-6">Pour plus d'infos, consulter la pièce jointe<i class="lnr lnr-paperclip" style="font-size: 30px" ></i>
...
function readpj(file){
  if(file.files&&file.files[0]){

    var reader=new FileReader();
    reader.onload=function(e){
      var pj = $('#pj').val();
        if (pj.substring(3,11) == 'fakepath') {
            pj = pj.substring(12);
        } // Remove c:\fake at beginning from localhost chrome
        $('#piece_jointe').html(pj+" <span id='enlever' style='color:red'>X<span>");
    }
    reader.readAsDataURL(file.files[0]);
  }
}
...
function readp_j(file){
  if(file.files&&file.files[0]){

    var reader=new FileReader();
    reader.onload=function(e){
      var p_j = $('#p_j').val();
        if (p_j.substring(3,11) == 'fakepath') {
            p_j = p_j.substring(12);
        } // Remove c:\fake at beginning from localhost chrome
        $('#pieceJointe').html(p_j+" <span id='enlever1' style='color:red'>X<span>");
    }
    reader.readAsDataURL(file.files[0]);
  }
}
...
// $('#enlever').click(function(){
 $(document).on('click','#enlever',function(){
  ...
  $(document).on('click','#enlever1',function(){
    $('#pieceJointe').html('');
    $('#p_j').val('');
...
function readimg1(file){
  if(file.files&&file.files[0]){

    var reader=new FileReader();
    reader.onload=function(e){
      $("#image_modif").attr('src',e.target.result);
    }
    reader.readAsDataURL(file.files[0]);
  }
}
...
$('.admin').mouseenter(function(){
 ...
 $('.'+id).show();
  // alert(id); 
  $.ajax({
                            url:"<?php echo base_url() ?>Actualite/edit",
                            method:"POST",
                            //async:false,
                            data: {id:id},
                            ...
                            success:function(data)
                                                {  
                                                     ...
                                                     $('#titre1').val(info[0]);
                                                     $('#description1').summernote('code',info[1]);
                                                     // $('#description1').val(info[1]);
                                                     // $('#nom_modif').val(info[0]);
                                                     // $('#nom_modif').val(info[0]);
                                                     $("#image_modif").attr('src',"<?=base_url('uploads/actualite/')?>"+info[2]);
                                                     $('#id').val(id);
                                                     $('#pieceJointe').html(info[3]+" <span id='enlever1' style='color:red'>X<span>");
                                                }
                });
});
...
$('.admin').mouseleave(function(){
 ...
 $(document).on('click','.delete',function(){
   var id=$(this).attr("id"); 
    // alert(id);
    $("#delete_modal").attr('href',"<?php echo base_url()?>actualites/Actualite/delete/"+id);
});
