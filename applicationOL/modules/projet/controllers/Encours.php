...
class Encours extends CI_Controller
{
    ...
    public function index(){
                $rapport=$this->Model->getRequete('SELECT* from projet r left join utilisateur u on r.FAIT_PAR=u.ID where STATUT=0 order by r.ID_RAPPORT DESC');
...
if($this->session->userdata('PROFIL')){
        ...
        // $data['option'].='<li><a href='.base_url("zone/Zone_localisation/details/").$localisation["LOCALISATION_ID"].'>Détails</li>'; 
         $data['option'].='<li style="text-align:center"><a href='.base_url("projet/Encours/update/").$val["ID_RAPPORT"].'><span class="lnr lnr-pencil"style="text-alin:center"></li>';
                                        ...
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-footer'>
                                                    <a class='btn btn-danger btn-md ' href='" . base_url('projet/Encours/delete/'.$val["ID_RAPPORT"]). "'>Supprimer</a>
    ...
    $template=array(
      ...
      if($this->session->userdata('PROFIL')){
        $this->table->set_heading('DATE','TITRE DU PROJET','FICHIER A TELECHARGER','AJOUTER PAR','action');
      }else
       $this->table->set_heading('DATE','TITRE DU PROJET','FICHIER A TELECHARGER');
    ...
    $datas['breadcrumb'] = $this->make_bread->output();
        ...
        $datas['error'] = "";
            $this->load->view('Encours_View', $datas);
...
$this->Model->create('projet',array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'STATUT'=>$_POST['statut'],'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
...
$this->session->set_flashdata($data);
  redirect(base_url('projet/Encours'));
        
    }
      public function delete($id){

$rapport=$this->Model->getOne('projet',array('ID_RAPPORT'=>$id));
$path=FCPATH. "uploads/rapport/".$rapport['FILE'];
      unlink($path);

    $this->Model->delete('projet',array('ID_RAPPORT'=>$id));
...
if($this->session->userdata('PROFIL')){
    ...
    $this->load->view('Encours_View1',$datas);
  }
...
// $data1=array('upload_data'=>$this->upload_pieceJointe->data());
$this->Model->update('projet',array("ID_RAPPORT"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'STATUT'=>$_POST['statut'],'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }else{
                $this->Model->update('projet',array("ID_RAPPORT"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'STATUT'=>$_POST['statut'],'FAIT_PAR'=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }
$this->session->set_flashdata($data);
  redirect(base_url('projet/Encours'));
