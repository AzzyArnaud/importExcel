...
class Etat_financier extends CI_Controller
{
    ...
    // }
    public function index(){
          $rapport=$this->Model->getRequete('SELECT* from rapport_etat_financier r left join utilisateur u on r.FAIT_PAR=u.ID order by r.ID_RAPPORT DESC');
   ...
   $this->load->view('Etat_View', $datas);
    }
...
$this->Model->create('rapport_etat_financier',array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
...
$this->session->set_flashdata($data);
  redirect(base_url('rapport/Etat_financier'));
        
    }
      public function delete($id){

$rapport=$this->Model->getOne('rapport_etat_financier',array('ID_RAPPORT'=>$id));
$path=FCPATH. "uploads/rapport/".$rapport['FILE'];
      unlink($path);

    $this->Model->delete('rapport_etat_financier',array('ID_RAPPORT'=>$id));
  ...
  public function update($id){
              $rapport=$this->Model->getRequete('SELECT* from rapport_devise r left join utilisateur u on r.FAIT_PAR=u.ID order by r.ID_RAPPORT DESC');
      $profil=$this->session->userdata('PROFIL');
...
$date=date_create($val['DATE']);
        ...
        $data[]='<a href="'.base_url().'uploads/rapport/'.$val['FILE'].'" download><span class="lnr lnr-download" style="color:#266C67;font-size:16px"></span>
                '.$val['TITRE'].' du '.date_format($date,"d-m-Y").'</a>';
if($this->session->userdata('PROFIL')){
        ...
        // $data['option'].='<li><a href='.base_url("zone/Zone_localisation/details/").$localisation["LOCALISATION_ID"].'>Détails</li>'; 
         $data['option'].='<li style="text-align:center"><a href='.base_url("rapport/Etat_financier/update/").$val["ID_RAPPORT"].'><span class="lnr lnr-pencil"style="text-alin:center"></li>';
                                        ...
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-footer'>
                                                    <a class='btn btn-danger btn-md ' href='" . base_url('rapport/Etat_financier/delete/'.$val["ID_RAPPORT"]). "'>Supprimer</a>
   ...
   $template=array(
      ...
      if($this->session->userdata('PROFIL')){
        $this->table->set_heading('DATE','FICHIER','AJOUTER PAR','action');
      }else
       $this->table->set_heading('DATE','FICHIER');
        $this->table->set_template($template);
        
    $datas['table']=$resultat;

    $rapport=$this->Model->getOne("rapport_devise",array("ID_RAPPORT"=>$id));
    $datas['profil']=$profil;
    $datas['rapport']=$rapport;
    $id=$this->session->userdata('ID');
    // $this->make_bread->add('Editer mon identification', "editer_identification/".$id, 1);
    ...
    $this->load->view('Devise_View1',$datas);
  }
...
// $data1=array('upload_data'=>$this->upload_pieceJointe->data());
$this->Model->update('rapport_etat_financier',array("ID_RAPPORT"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
$this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier un rapport annuel","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }else{
                $this->Model->update('rapport_etat_financier',array("ID_RAPPORT"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FAIT_PAR'=>$this->session->userdata('ID')));
                $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier un rapport annuel","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }
$this->session->set_flashdata($data);
