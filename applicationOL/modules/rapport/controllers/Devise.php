...
if($this->session->userdata('PROFIL')){
        ...
        // $data['option'].='<li><a href='.base_url("zone/Zone_localisation/details/").$localisation["LOCALISATION_ID"].'>Détails</li>'; 
         // $data['option'].='<li style="text-align:center"><a href='.base_url("rapport/Devise/update/").$val["ID_RAPPORT"].'><span class="lnr lnr-pencil"style="text-alin:center"></li>';
                                        ...
                                        <div class='modal-dialog'>
                                            <div class='modal-content'>
                                                ...
                                                <div class='modal-footer'>
                                                    <a class='btn btn-danger btn-md ' href='" . base_url('rapport/Devise/delete/'.$val["ID_RAPPORT"]). "'>Supprimer</a>
   ...
   $this->load->view('Devise_View', $datas);
    }
...
$this->Model->create('rapport_devise',array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
...
$this->session->set_flashdata($data);
  redirect(base_url('rapport/Devise'));
...
$path=FCPATH. "uploads/rapport/".$rapport['FILE'];
    ...
    $this->Model->delete('rapport_devise',array('ID_RAPPORT'=>$id));
...
// $data1=array('upload_data'=>$this->upload_pieceJointe->data());
$this->Model->update('rapport_devise',array("ID_RAPPORT"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FILE'=>$file_name,'FAIT_PAR'=>$this->session->userdata('ID')));
$this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier un rapport annuel","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }else{
                $this->Model->update('rapport_devise',array("ID_RAPPORT"=>$id),array('TITRE'=>$titre,'DATE'=>date('Y-m-d H:i:s'),'FAIT_PAR'=>$this->session->userdata('ID')));
                $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier un rapport annuel","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Enregistrement avec succès</div>';
              }
$this->session->set_flashdata($data);
