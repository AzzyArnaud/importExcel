...
class Stages extends CI_Controller
{
   ...
   if ($this->uri->segment(4)) {
    ...
    $query =$this->db->get('stage','10',$segment);
    $datas['emploi']=$query->result();
    $datas['breadcrumb'] = $this->make_bread->output();
    // $query=$this->db->get('emploi');

    $total=$this->Model->getList('stage',array("EXPIRATION>"=>date("Y-m-d H:i:s")));

    $config=array();
    $config['base_url']=base_url()."opportunite/Stages/index";
    ...
    $this->pagination->initialize($config);
        $datas['error'] = "";
        $datas['error'] = "";
            $this->load->view('Stages_View', $datas);
  ...
  public function edit(){
    ...
    $info=$this->Model->getOne('stage',array('ID'=>$id));
    // echo $id;
  ...
  $config['upload_path']='./uploads/Stages/';
...
if (!empty($expiration)) {
      $this->Model->create('stage',array('TITRE'=>$titre,'DESCRIPTION'=>$description,'DATE'=>date('Y-m-d H:i:s'),'EXPIRATION'=>$expiration,'PIECE_JOINTE'=>$data['upload_data']['file_name'],'AJOUTER_PAR'=>$this->session->userdata('ID')));
      $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Ajouter une publication de stage","USER_ID"=>$this->session->userdata('ID')));
  ...
  redirect(base_url('opportunite/Stages'));
//   }else{
...
$data=array('upload_data'=>$this->upload->data());
      date_default_timezone_set('Africa/Bujumbura');
 $emploi=$this->Model->getOne('stage',array('ID'=>$id));


      $this->Model->update('stage',array('ID'=>$id),array('TITRE'=>$titre,'DESCRIPTION'=>$description,'DATE'=>date('Y-m-d H:i:s'),'EXPIRATION'=>$expiration,'PIECE_JOINTE'=>$data['upload_data']['file_name'],'AJOUTER_PAR'=>$this->session->userdata('ID')));
      $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une publication de stage","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Modification avec succès</div>';

            if (!empty($emploi['PIECE_JOINTE'])) {
              $path=FCPATH. "uploads/Stages/".$emploi['PIECE_JOINTE'];
    unlink($path);
            }
  ...
  redirect(base_url('opportunite/Stages'));
     ...
     $this->Model->update('stage',array('ID'=>$id),array('TITRE'=>$titre,'DESCRIPTION'=>$description,'DATE'=>date('Y-m-d H:i:s'),'EXPIRATION'=>$expiration,'AJOUTER_PAR'=>$this->session->userdata('ID')));
     $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une publication de stage","USER_ID"=>$this->session->userdata('ID')));
...
$emploi=$this->Model->getOne('stage',array('ID'=>$id));
    ...
    $this->Model->delete('stage',array('ID'=>$id));
    $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Supprimer une publication de stage","USER_ID"=>$this->session->userdata('ID')));
...
$mot_cle=$this->input->post('mot_cle');
   ...
   if ($this->uri->segment(4)) {
    ...
    $total=$this->Model->getList('stage');

    $config=array();
    $config['base_url']=base_url()."Stages/index";
    $config['total_rows']=sizeof($total);
    $config['per_page']=10;
    $config['uri_segment']=4;
    $config['use_page_numbers']=TRUE;
    ...
    $this->load->view('Recherche_stage_view',$data);
  }
}
?>
