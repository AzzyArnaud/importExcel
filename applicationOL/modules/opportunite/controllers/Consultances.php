...
class Consultances extends CI_Controller
{
   ...
   if ($this->uri->segment(4)) {
    ...
    $query =$this->db->get('consultance','10',$segment);
    $datas['emploi']=$query->result();
    $datas['breadcrumb'] = $this->make_bread->output();
    // $query=$this->db->get('emploi');

    // $total=$this->Model-> getList('consultance',array("EXPIRATION>"=>date("Y-m-d H:i:s")));
    $total=$this->Model-> getList('consultance');

    $config=array();
    $config['base_url']=base_url()."opportunite/Consultances/index";
    ...
    $this->pagination->initialize($config);
        ...
        $datas['error'] = "";
        $datas['error'] = "";
            $this->load->view('Consultances_View', $datas);
  ...
  public function edit(){
    ...
    $info=$this->Model->getOne('consultance',array('ID'=>$id));
    // echo $id;
  ...
  $config['upload_path']='./uploads/Consultances/';
...
if (!empty($expiration)) {
      $this->Model->create('consultance',array('TITRE'=>$titre,'DESCRIPTION'=>$description,'DATE'=>date('Y-m-d H:i:s'),'EXPIRATION'=>$expiration,'PIECE_JOINTE'=>$data['upload_data']['file_name'],'AJOUTER_PAR'=>$this->session->userdata('ID')));
          $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Ajouter une publication de consultance","USER_ID"=>$this->session->userdata('ID')));
  ...
  redirect(base_url('opportunite/Consultances'));
//   }else{
...
$data=array('upload_data'=>$this->upload->data());
      date_default_timezone_set('Africa/Bujumbura');
 $emploi=$this->Model->getOne('consultance',array('ID'=>$id));


      $this->Model->update('consultance',array('ID'=>$id),array('TITRE'=>$titre,'DESCRIPTION'=>$description,'DATE'=>date('Y-m-d H:i:s'),'EXPIRATION'=>$expiration,'PIECE_JOINTE'=>$data['upload_data']['file_name'],'AJOUTER_PAR'=>$this->session->userdata('ID')));
      $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une publication de consultance","USER_ID"=>$this->session->userdata('ID')));
             $data['message']='<div class="alert alert-success text-center"> Modification avec succès</div>';
             if (!empty($emploi['PIECE_JOINTE'])) {
     $path=FCPATH. "uploads/Consultances/".$emploi['PIECE_JOINTE'];
    unlink($path);
  }
    $this->session->set_flashdata($data);
  redirect(base_url('opportunite/Consultances'));
  }else{
     $this->Model->update('consultance',array('ID'=>$id),array('TITRE'=>$titre,'DESCRIPTION'=>$description,'DATE'=>date('Y-m-d H:i:s'),'EXPIRATION'=>$expiration,'AJOUTER_PAR'=>$this->session->userdata('ID')));
     $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Modifier une publication de consultance","USER_ID"=>$this->session->userdata('ID')));
...
$emploi=$this->Model->getOne('consultance',array('ID'=>$id));
    ...
    $this->Model->delete('consultance',array('ID'=>$id));
    $this->Model->create("historique_navigation",array("DESCRIPTION"=>"Supprimer une publication de consultance","USER_ID"=>$this->session->userdata('ID')));
...
$mot_cle=$this->input->post('mot_cle');
   ...
   if ($this->uri->segment(4)) {
    ...
    $this->load->view('Recherche_consultance_view',$data);
  }
}
?>
