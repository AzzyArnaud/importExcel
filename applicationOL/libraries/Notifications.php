...
function send_mail($emailTo = array(), $subjet, $cc_emails = array(), $message, $attach = array()) {
        ...
        $this->CI->email->from('noc@mediabox.bi', 'BIJE');
   ...
   {     
       ...
       // $config['priority'] = '1';
        ...
        $this->CI->email->from('helpdesk_comesa@mediabox.bi', 'BIJE Projet');
        $this->CI->email->to($emailTo);
        //$this->CI->email->bcc('ismael@mediabox.bi');
...
public function send_sms($string_tel = NULL,$string_msg)
   {
        ...
        $header = array();                 
        $header [0] = 'Authorization:Token 9108b54d0c57ce3b170faeb288f6319d0b498686';  //pas d'espace entre Authori et : et Token
        $header [1] = 'Content-Type:application/json';
        $curl = curl_init();
