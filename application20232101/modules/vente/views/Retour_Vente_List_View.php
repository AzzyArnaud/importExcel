      ...
      <div class="container-fluid">
        ...
        <tbody>
          ...
          foreach ($vente as $key) 
         {
          ...
          $det_medicam = $this->Model->getRequete('SELECT NOM_PRODUIT, PRIX_UNITAIRE, BARCODE FROM `vente_detail` JOIN saisie_produit ON saisie_produit.ID_PRODUIT=vente_detail.ID_PRODUIT JOIN req_barcode ON req_barcode.ID_BARCODE = vente_detail.ID_BARCODE WHERE `ID_VENTE` = '.$key['ID_VENTE'].'');
          ...
          $details .= '<tr><td>'.$value['NOM_PRODUIT'].'</td><td>1</td><td>'.$value['PRIX_UNITAIRE'].'</td><td>'.$value['BARCODE'].'</td><td><a class="btn btn-danger btn-sm" href="'.base_url('vente/Retour_Vente/delete_items/'.$key['ID_VENTE'].'').'" target="_blank" role="button"><i class="fa fa-minus-circle" aria-hidden="true"></i> Enlever </a></td></tr>';
          }
          $details .='</table>';
